# Mechanism knockout analyses

## Overview

Mechanism knockouts allow users to evaluate how individual model
components affect simulated community assembly. Here, we switch off
individual modules (e.g., spatial layering, host sanctions) to quantify
each mechanism’s contribution relative to the baseline configuration
modeling (BCM).

Run a set of reduced model configurations to test the necessity of
specific intrinsic mechanisms. Each entry modifies one key mechanism in
the baseline configuration modeling (BCM) while keeping other parameters
fixed. we need remove the figs labeled as “dropped” from the final
dataset.
