...
Ecologists have long recognized that natural communities exhibit large variability. `figsimR` operationalizes a mechanistic null workflow: we calibrate a Full Intrinsic Model (FIM) to reproduce empirical means, then quantify the Variation Gap—the residual dispersion that not captured by the current FIM configuration. Finally, we use mechanism knockout experiments to dissect which internal processes maintain the theoretical structure.
...
Framing & scope. In this vignette, the “Variation Gap” is defined strictly as model–data residual variation under the current FIM configuration. It is not a claim that ecological variability is inherently “extrinsic” or unknowable. Many well-studied drivers—e.g., environmental/landscape heterogeneity, dispersal, demographic/environmental stochasticity, evolutionary change—can be added as explicit modules and may reduce or eliminate the gap. Here we hold these out of scope to (i) delineate the explanatory envelope of intrinsic mechanisms, and (ii) provide a transparent baseline for subsequent extensions.
...
# 3. Act I — Build the theoretical world (FIM)
...
## 3.2 Parameter ranges for FIM calibration
...
## 3.4 Simulate the FIM with best parameters
...
# 4. Act II — Quantify the model–data residual variance (‘Variation Gap’)
...
**Interpretation.** If the simulated cloud matches means but exhibits smaller multivariate dispersion than observed, the Variation Gap is present (intrinsic model underestimates real variability). This result diagnoses a limit of the present model, not of ecological theory writ large. The magnitude and even the presence of the gap are model-, metric-, and dataset-dependent (e.g., choice of distance, sample size, site heterogeneity). As additional, well-supported processes are incorporated, the gap is expected to shrink.

# 5. Act III — Dissect internal mechanisms (knockouts)

We switch off individual modules (e.g., spatial layering, host sanctions) to quantify each mechanism’s contribution relative to the FIM.
...
# 6. Act IV — Probing the Host Sanction Threshold
...
Notes: (i) Parasitoid eggs are not counted toward the ovule-occupation total because they do not occupy ovules directly. (ii) If your function still uses the legacy argument name host_sanction, use it; otherwise use the corrected host_sanction_threshold.
...
* **Extensibility.** Treat the FIM vignette as a baseline. If a project requires environmental or spatial structure, add those modules and re-estimate the gap.
